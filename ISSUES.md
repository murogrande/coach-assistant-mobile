...
## Issue #21: Multi-Provider Support in API Client

**Labels**: `frontend`, `api`, `high-priority`

**Description**:

Extend the API client to support the `provider` parameter when generating analyses,
and add a method to list all analyses for a given week (across providers).

**Tasks**:
- [ ] Update `generate_analysis(week_start_date, provider="openai")` to include `provider` in the POST body
- [ ] Add `get_analyses_for_week(week_start_date: str) -> List[Dict]` method:
  - [ ] `GET /api/analysis/?week_start_date={date}` — returns all analyses for the week (all providers)
- [ ] Update `get_latest_analysis(provider: str = None) -> Optional[Dict]`:
  - [ ] If `provider` given, call `GET /api/analysis/latest/?provider={provider}`
  - [ ] If not given, call `GET /api/analysis/latest/` as before
- [ ] Add unit tests for all three updated/new methods (mock responses)

**Acceptance Criteria**:
- `generate_analysis()` sends `provider` in request body
- `get_analyses_for_week()` returns list of analyses for a week
- `get_latest_analysis()` supports optional provider filter
- All new methods covered by tests

---

## Issue #22: Provider Selector UI on Analysis Screen

**Labels**: `frontend`, `ui`, `high-priority`

**Description**:

Add a provider selector to the analysis screen so the user can choose which AI provider
to use before generating. Show a provider badge on the displayed analysis so it is always
clear which model produced the current result.

**Tasks**:
- [ ] Add provider chips row below the week selector (in BG area, above scroll):
  - [ ] Three chips: "OpenAI", "Claude", "Local (Qwen3)"
  - [ ] Single-select — selected chip is filled/highlighted, others outlined
  - [ ] Default selected: "OpenAI"
  - [ ] Store selection in `_selected_provider` attribute (`"openai"` / `"anthropic"` / `"local"`)
- [ ] Pass `_selected_provider` to `api_client.generate_analysis()` (Issue #10 / #21)
- [ ] Add provider badge to the analysis display area:
  - [ ] Small chip/label showing e.g. "Generated by Claude" or "Generated by Local (Qwen3)"
  - [ ] Read from `provider` field in the API response
  - [ ] Visible only when analysis is displayed (hidden in empty state)
- [ ] Add unit tests for:
  - [ ] Chips render correctly
  - [ ] `_selected_provider` updates on chip tap
  - [ ] Provider badge shows/hides correctly

**Acceptance Criteria**:
- User can select provider before generating
- Selected provider is highlighted clearly
- Provider badge always visible when analysis is shown
- Default provider is OpenAI

---

## Issue #23: Compare Analyses from Multiple Providers

**Labels**: `frontend`, `ui`, `medium-priority`

**Description**:

When multiple analyses exist for the same week (generated by different providers),
allow the user to switch between them to compare coaching insights side by side.

**Tasks**:
- [ ] On `on_enter` (screen focus), call `api_client.get_analyses_for_week()` for the current week
- [ ] If multiple analyses returned:
  - [ ] Show a provider toggle row above the section cards (e.g. "OpenAI | Claude | Local")
  - [ ] Tapping a provider loads and displays that analysis in the section cards
  - [ ] Highlight the active provider in the toggle
- [ ] If only one analysis for the week: hide the toggle, display it directly
- [ ] If no analyses for the week: show empty state as usual
- [ ] When week changes (via `<` / `>` buttons), reload analyses for the new week
- [ ] Add unit tests:
  - [ ] Toggle renders when multiple analyses present
  - [ ] Toggle hidden when only one analysis
  - [ ] Switching provider updates displayed content

**Acceptance Criteria**:
- Provider toggle appears only when multiple analyses exist for the week
- Switching providers updates all section cards
- Week navigation reloads analyses for the new week
- Empty state shown when no analyses exist for selected week
...
## Milestone: Multi-Provider Analysis

**Target**: Complete issues #21-23 (mobile) + #39-42 (backend)
**Dependency**: Requires backend issues #39-42 to be completed first
